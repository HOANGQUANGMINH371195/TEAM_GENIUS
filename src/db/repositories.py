    ...
    async def search_legal_unit_phrases(self, query: str, *, dataset_id: str, limit: int = 12) -> list[RetrievalResult]:
        ...
        result = await self.session.execute(text("""
            ...
            WHERE u.dataset_id=:dataset_id AND NOT d.is_external AND COALESCE((d.payload->'metadata'->>'answer_ready')::boolean,FALSE) IS TRUE
              ...
              AND EXISTS (SELECT 1 FROM unnest(CAST(:phrases AS text[])) p(phrase)
                          WHERE to_tsvector('simple',coalesce(u.label,'')||' '||coalesce(u.heading,'')||' '||coalesce(u.text,'')) @@ phraseto_tsquery('simple',p.phrase))
            ORDER BY u.source_start, u.unit_id LIMIT :limit
        """), {"dataset_id": dataset_id, "phrases": phrases, "limit": max(1,min(limit,24))})
