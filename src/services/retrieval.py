...
def requires_clause_expansion(query: str) -> bool:
    ...
    return bool(
        re.search(
            r"\b(?:bao\s+nhiêu|bao\s+lâu|khi\s+nào|từ\s+khi\s+nào|trường\s+hợp\s+nào|điều\s+kiện\s+nào|quyền\s+lợi\s+gì|mức\s+hưởng|được\s+hưởng|có\s+được)\b|\b(?:thay\s+đổi|tính|xác\s+định|phân\s+biệt|khác\s+nhau)\b[\s\S]{0,40}\b(?:thế\s+nào|ra\s+sao)\b|\bcó\b[\s\S]{0,100}\bkhông\b|%",
            normalized,
        )
    )
