...
| Typed BHYT fact contract | `src/domain/facts.py`, `src/services/fact_recognizer.py`, `legal_facts` migration, release-validated importer, `Neo4jGraphStore.upsert_legal_facts`/`bounded_typed_ppr`, accepted-subject relational route, canonical unit hydration, bounded-query acceptance tests. Local Neo4j `5.26.29-community` is healthy, but currently contains only `Document` labels across three historical projections; no typed fact labels have been imported. | repository projection/walk contract implemented; human review/recognition quality, typed projection load, and live parity attestation pending |
