...
| Exact/lexical/dense/PageIndex retrieval | `src/services/chat.py`, `src/db/repositories.py`, current-authority and query-scope filters, release-scoped cache policy | implemented with generic scope/currentness filtering and synthesis-only high-risk answers; production latency gate not proven |
