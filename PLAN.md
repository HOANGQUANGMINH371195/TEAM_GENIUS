...
| Typed BHYT fact contract | `docs/data/typed-bhyt-ontology.json`, `src/domain/ontology.py`, `src/domain/facts.py`, `src/services/fact_recognizer.py`, `legal_facts` migration, release-validated importer/exporter, `Neo4jGraphStore.upsert_legal_facts`/`bounded_typed_ppr`, accepted-subject relational route, canonical unit hydration, bounded-query acceptance tests. Migration `20260827_typed_legal_facts` is now applied on managed PostgreSQL with RLS; active release remains `snapshot-c439...`. Both managed and local exports currently have `accepted_facts=0`; managed Neo4j Aura `5.27-aura` has 1.901 active `Document` nodes and no typed-fact labels. | ontology/loader contract implemented; human review/recognition quality and typed projection load remain pending; managed connectivity/parity verified |
