...
| Typed BHYT fact contract | `docs/data/typed-bhyt-ontology.json`, `src/domain/ontology.py`, `src/domain/facts.py`, `src/services/fact_recognizer.py`, `legal_facts` migration, release-validated importer/exporter, `Neo4jGraphStore.upsert_legal_facts`/`bounded_typed_ppr`, accepted-subject relational route, canonical unit hydration, bounded-query acceptance tests. Local export for `snapshot-c439751724ab7f10` is reproducibly empty (`accepted_facts=0`), and Neo4j `5.26.29-community` currently contains only `Document` labels across three historical projections; no typed fact labels have been imported. | ontology/loader contract implemented; human review/recognition quality, typed projection load, and live parity attestation pending |
