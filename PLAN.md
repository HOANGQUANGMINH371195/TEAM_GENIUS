...
| Claim uncertainty contract | `LegalClaim.uncertainty` with faithfulness/factuality/completeness fields, `eval/calibration.py` metrics, and independent-panel validator requiring complete labels from at least two reviewers | partial: reviewed calibration labels/model fitting pending |
...
| Typed BHYT fact contract | `docs/data/typed-bhyt-ontology.json`, `src/domain/ontology.py`, `src/domain/facts.py`, `src/services/fact_recognizer.py`, `legal_facts` migration, release-validated importer/exporter, `Neo4jGraphStore.upsert_legal_facts`/`bounded_typed_ppr`, accepted-subject relational route, canonical unit hydration, bounded-query acceptance tests. Migration `20260827_typed_legal_facts` is applied on managed PostgreSQL with RLS; both managed and local exports currently have `accepted_facts=0`. Direct managed Neo4j connectivity is healthy (`Neo4j/5.27-aura`, enterprise edition), but it currently contains two release-scoped `Document` snapshots (1,901 nodes each), no typed-fact labels, and no typed-fact relationships. The local Docker graph is Community 5.26.29 with three local/release snapshots. Projection now rejects unreviewed or unanchored facts in the adapter as a defense-in-depth gate. | ontology/loader/safety contract implemented; human review/recognition quality and typed projection load remain pending; graph connectivity passes but live typed projection/parity is not proven |
| Source/release parity | The default current builder correctly fails closed because it rebuilds a different fingerprint (`snapshot-037cca…`, `38,316`/`14,968`) than the active release. Re-running with the exact builder recorded in `docs/data/release-lock-snapshot-c439751724ab7f10.json` (`source_commit=1b98f44`) now **passes** fingerprint, canonical counts, PostgreSQL/Qdrant counts and hashes, and Neo4j identity/edge parity. The report still warns that Neo4j retains one stale second release snapshot. The prior `live_parity.json` is stale evidence. | exact-builder parity verified; stale projection cleanup and immutable artifact retention remain required before production promotion |
