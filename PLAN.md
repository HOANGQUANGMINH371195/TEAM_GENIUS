...
|---|---|---|
| Local/managed readiness | Docker Desktop `local-full` profile started from `/home/minh/projects/team-Vin-genius/.env`; local Postgres, Qdrant, Neo4j, Redis, API and web containers healthy. Additive migrations `20260824_document_lexical_index` and `20260827_typed_legal_facts` applied; API/web/migrate images rebuilt from current source; `make build` and `make check ENV_FILE=.../.env` passed; 20-request local `/ready` smoke p95 124.5ms, all 200. Read-only Render API metadata on 2026-08-28 confirms service `medipay-api` is Docker/starter/Oregon/main with `/health`; no `medipay-research-worker` exists, and the only listed Render Redis is free but suspended, so the durable worker is not provisioned. | local/managed API readiness is evidenced; durable worker/Redis provisioning and authenticated managed latency/outage/rollback attestation remain external |
