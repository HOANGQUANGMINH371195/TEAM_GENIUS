...
export default function AdminIndexPage() {
  ...
  return <main className={`admin-dashboard${loading && data ? " is-refreshing" : ""}`}>
    <header className="admin-dashboard-hero"><div><h1>Nhìn rõ hệ thống đang chạy</h1><p className="admin-dashboard-intro">Số liệu vận hành lấy trực tiếp từ Langfuse.</p></div><button className="admin-dashboard-refresh" type="button" onClick={() => void load()} disabled={loading}><span className={loading ? "is-spinning" : ""}>↻</span>{loading ? "Đang tải..." : "Làm mới"}</button></header>
